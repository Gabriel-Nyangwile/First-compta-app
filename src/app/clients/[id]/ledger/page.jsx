...
export default function ClientLedgerPage({ params }) {
  ...
  const groupedRows = () => {
    if (!data?.rows) return [];
    const arr = []; let currentRef=null; let bucket=[];
    for (const r of data.rows) {
      if (r.invoiceRef !== currentRef) { if (bucket.length) arr.push({ref:currentRef, rows:bucket}); currentRef=r.invoiceRef; bucket=[r]; } else bucket.push(r);
    }
    ...
    <main className="min-h-screen pt-24 px-6 bg-gray-50">
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        <div className="flex items-center gap-4">
          ...
          <Link href={`/clients/${id}/lettering`} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded text-sm">Lettrage</Link>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white p-4 rounded shadow border text-sm space-y-3">
            ...
            <div className="pt-4 space-y-2 text-xs border-t">
              ...
              <label className="flex items-center gap-2"><input type="checkbox" checked={uiPrefs.highlightPayments} onChange={()=>toggle('highlightPayments')} /> Mettre en évidence encaissements</label>
                ...
                <tbody>
                  ...
                  {!loading && data && groupedRows().map(group => {
                    ...
                    return group.rows.map((r, idx) => {
                      ...
                      const rowClasses = ['border-b','hover:bg-gray-50','transition-colors'];
                      ...
                      return (
                        <tr key={r.id} className={rowClasses.join(' ')}>
                          ...
                          <td className="px-2 py-1" title={r.linesPreview || undefined}>
                            ...
                            {highlightPayment && <span className="ml-2 inline-block px-1.5 py-0.5 rounded bg-emerald-600 text-white text-[10px]">ENCAISSEMENT</span>}
