...
import { releaseOrder } from "@/lib/production";
...
export async function POST(request, context) {
  try {
    ...
    const order = await releaseOrder({ id, companyId });
    return NextResponse.json({ order });
  } catch (error) {
    return NextResponse.json({ error: error.message || "Erreur lancement ordre." }, { status: 400 });
  }
}
