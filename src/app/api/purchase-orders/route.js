...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Erreur récupération bons de commande.' }, { status: 500 });
  }
}
...
export async function POST(request) {
  try {
    ...
    const normLines = lines.map((l, idx) => {
      ...
      if (!productId || isNaN(orderedQty) || orderedQty <= 0 || isNaN(unitPrice) || unitPrice < 0) {
        throw new Error(`Ligne ${idx+1} invalide.`);
      }
      const vatRate = l.vatRate != null && l.vatRate !== '' ? Number(l.vatRate) : null;
      if (vatRate != null && (isNaN(vatRate) || vatRate < 0)) throw new Error(`Taux TVA invalide ligne ${idx+1}`);
      return {
        productId,
