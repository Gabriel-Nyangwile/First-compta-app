...
export async function POST(request) {
  try {
    ...
    if (!category || typeof category !== 'string') return NextResponse.json({ error: 'Category requis' }, { status: 400 });
    if (!categoryDescription || typeof categoryDescription !== 'string') return NextResponse.json({ error: 'categoryDescription requis' }, { status: 400 });

    const parsedSalary = legalSalary != null ? Number(legalSalary) : null;
    if (parsedSalary == null || Number.isNaN(parsedSalary)) return NextResponse.json({ error: 'legalSalary invalide' }, { status: 400 });
