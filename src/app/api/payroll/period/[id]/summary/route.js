...
export async function GET(req, { params }) {
  ...
  if (format === 'csv') {
    if (section === 'liabilities') {
      const headers = ['processingCurrency','fiscalCurrency','fxRate','code','label','group','total','settled','remaining','settlementStatus','paymentFlowReady'];
      const rows = summary.liabilities.map((item) => [
        currencyContext.processingCurrency,
        currencyContext.fiscalCurrency,
        currencyContext.fxRate ?? '',
        item.code,
        item.label,
        item.group,
    ...
    const headers = ['processingCurrency','fiscalCurrency','fxRate','ref','employeeNumber','employeeName','gross','net','settledAmount','remainingAmount','isSettled','cnssEmployee','iprTax','cnssEmployer','onem','inpp','overtime','employerCharges','linesCount'];
    const rows = summary.employees.map(e => [currencyContext.processingCurrency,currencyContext.fiscalCurrency,currencyContext.fxRate ?? '',e.ref,e.employeeNumber || '',e.employeeName,e.gross.toFixed(2),e.net.toFixed(2),e.settledAmount.toFixed(2),e.remainingAmount.toFixed(2),e.isSettled ? 'yes' : 'no',e.cnssEmployee.toFixed(2),e.iprTax.toFixed(2),e.cnssEmployer.toFixed(2),e.onem.toFixed(2),e.inpp.toFixed(2),e.overtime.toFixed(2),(e.employerCharges).toFixed(2),e.linesCount]);
    const csv = [headers.join(','), ...rows.map(r => r.map(v => '"'+String(v).replace(/"/g,'""')+'"').join(','))].join('\n');
