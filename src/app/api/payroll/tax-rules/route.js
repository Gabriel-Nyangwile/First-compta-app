...
export async function GET(request) {
  ...
  const items = await prisma.taxRule.findMany({ where: { companyId }, orderBy: { code: 'asc' }, take: 100 });
...
export async function POST(req) {
  ...
  try {
    ...
    const existing = await prisma.taxRule.findUnique({ where: { companyId_code: { companyId, code: v.data.code } } });
    if (existing) return new Response(JSON.stringify({ ok:false, error:'code.exists' }), { status:409 });
    const created = await prisma.taxRule.create({ data: { ...v.data, companyId } });
