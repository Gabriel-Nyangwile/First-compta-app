...
export async function GET(req) {
  ...
  draw(`Devise(s) traitement: ${annual.currencySummary.processingCurrencies.join(', ') || '-'} | Devise(s) fiscale(s): ${annual.currencySummary.fiscalCurrencies.join(', ') || '-'}`, 9);
  if (annual.currencySummary.mixedProcessingCurrencies || annual.currencySummary.missingFxMonths.length) {
    draw(`Alerte devise: mixte=${annual.currencySummary.mixedProcessingCurrencies ? 'oui' : 'non'} | taux manquants=${annual.currencySummary.missingFxMonths.map(m => `${m.month}/${m.periodRef}`).join(', ') || '-'}`, 8);
  }
  ...
  y -= 2;
  draw('Mois:', 12);
  const header = ['Mois','Dev','Taux','Brut','CorrBrut','Net','CorrNet','CNSS Sal','IPR','CNSS Emp','ONEM','INPP','Charges Emp','HS','Cumul Brut','Cumul Net'];
  const colWidths = [35,35,45,55,50,55,50,60,50,60,45,45,70,40,65,65];
  let x = margin;
  const startY = y;
  ...
  for (const m of annual.months) {
    x = margin;
    const row = [m.month, m.processingCurrency || '-', m.fxRate ?? '-', m.grossTotal.toFixed(0), m.grossNegative.toFixed(0), m.netTotal.toFixed(0), m.netNegative.toFixed(0), m.cnssEmployeeTotal.toFixed(0), m.iprTaxTotal.toFixed(0), m.cnssEmployerTotal.toFixed(0), m.onemTotal.toFixed(0), m.inppTotal.toFixed(0), m.employerChargesTotal.toFixed(0), m.overtimeTotal.toFixed(0), m.ytdGross.toFixed(0), m.ytdNet.toFixed(0)];
