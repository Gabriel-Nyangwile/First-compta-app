...
export async function GET(req){
  ...
  const [totalEmployees, activeEmployees, statusGroups, contractGroups, hiresMonth, exitsMonth, hiresYtd, exitsYtd, activeWithHireDates, activeWithBirthDates] = await Promise.all([
    ...
    prisma.employee.count({ where:{ companyId, status:'ACTIVE', OR:[{ endDate:null }, { endDate:{ gt: now } }] } }),
    ...
    prisma.employee.findMany({ where:{ companyId, status:'ACTIVE', hireDate:{ not:null, lte: now }, OR:[{ endDate:null }, { endDate:{ gt: now } }] }, select:{ hireDate:true } }),
    prisma.employee.findMany({ where:{ companyId, status:'ACTIVE', birthDate:{ not:null, lte: now }, OR:[{ endDate:null }, { endDate:{ gt: now } }] }, select:{ birthDate:true } }),
  ]);
  ...
  const activeAtYearStart = await prisma.employee.count({ where:{ companyId, status:'ACTIVE', hireDate:{ lte: yearStart }, OR:[{ endDate:null }, { endDate:{ gt: yearStart } }] } });
  const activeAtMonthStart = await prisma.employee.count({ where:{ companyId, status:'ACTIVE', hireDate:{ lte: monthStart }, OR:[{ endDate:null }, { endDate:{ gt: monthStart } }] } });
  ...
  const [monthPayslips, ytdPayslips] = await Promise.all([
    prisma.payslip.findMany({ where:{ period:{ companyId, month, year } }, select:{ grossAmount:true, netAmount:true } }),
    prisma.payslip.findMany({ where:{ period:{ companyId, year } }, select:{ grossAmount:true, netAmount:true } }),
  ]);
  ...
  ws.columns = [ { header:'Cle', key:'k', width:30 }, { header:'Valeur', key:'v', width:22 } ];
  ws.addRows([
