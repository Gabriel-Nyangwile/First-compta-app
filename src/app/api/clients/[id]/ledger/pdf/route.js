...
export async function GET(req, { params }) {
  ...
  const ledger = await getClientLedger({ id, companyId, dateStart, dateEnd, includeDetails });
  ...
  if (y < 70) { page = pdf.addPage(A4); pages.push(page); y = 732; drawHeader(page, font, 'Grand Livre Client (suite)', info); drawTableHeader(page, font, y); y -= 22; }
    ...
    if (y < 70) { page = pdf.addPage(A4); pages.push(page); y = 732; drawHeader(page, font, 'Grand Livre Client (suite)', info); }
    ...
    pages.forEach((p, idx) => drawFooter(p, { font, pageNumber: idx + 1, totalPages: pages.length, legal: 'Grand livre client' }));
