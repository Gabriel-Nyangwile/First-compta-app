      ...
      <section className="bg-white border rounded p-4 space-y-3">
        <form method="get" className="flex flex-wrap gap-4 items-end text-xs">
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            <label className="inline-flex items-center gap-1">
              <input type="checkbox" name="exceeded" value="1" defaultChecked={exceededOnly} /> <span>Montants supérieurs au restant dû</span>
            </label>
          </div>
          ...
          <table className="min-w-full text-xs border">
            <thead className="bg-slate-100">
              <tr className="text-left">
                ...
                <th className="px-2 py-1">Pièce liée</th>
                <th className="px-2 py-1">Solde restant</th>
              ...
              {rows.map(r => {
                ...
                return (
                  <tr key={r.id} className={`border-t hover:bg-slate-50 ${exceeded ? 'bg-red-50/50' : ''}`}>
                    ...
                    <td className="px-2 py-1">{DOC_TYPE_LABELS[r.docType] || r.docType}</td>
                    <td className="px-2 py-1">{FLOW_LABELS[r.flow] || r.flow}</td>
                    ...
                    <td className="px-2 py-1 flex items-center gap-2">
                      ...
                      <Link href={`/authorizations/${r.id}`} className="text-blue-600 underline">Consulter</Link>
                    </td>
                  </tr>
                );
              })}
              {!rows.length && (
                <tr><td colSpan={10} className="px-2 py-6 text-center text-slate-500">Aucune autorisation de trésorerie</td></tr>
              )}
            </tbody>
          </table>
        </div>
