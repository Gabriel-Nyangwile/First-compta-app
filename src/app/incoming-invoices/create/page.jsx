...
const AccountAutocomplete = dynamic(
  () => import("@/components/AccountAutocomplete"),
  { ssr: false }
);
...
export default function CreateIncomingInvoicePage() {
  ...
  useEffect(() => {
    ...
    // Charger uniquement les BC approuvés (ou partiellement / reçus mais pas clos)
    fetch(
      "/api/purchase-orders?supplierId=" + encodeURIComponent(form.supplierId)
    )
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d))
          setPurchaseOrders(
            d.filter((po) => ["RECEIVED", "CLOSED"].includes(po.status))
          );
      })
      .catch(() => {});
  }, [form.supplierId]);
    ...
    async function loadPOLines() {
      ...
      try {
        ...
        const rawLines = (po.lines || []).map((l) => ({
          ...
          remainingQty: Number(
            (Number(l.orderedQty) - Number(l.receivedQty)).toFixed(3)
          ),
        ...
        // Fallback: si toutes les quantités reçues sont nulles et mode 'received', on bascule sur 'ordered'
        if (
          poQuantityMode === "received" &&
          selected.every((l) => (l.qty || 0) <= 0)
        ) {
          selected = rawLines.map((l) => ({
            ...l,
            qty: l.orderedQty,
            _fallbackFromReceived: true,
          }));
        }
        // Fallback additionnel: si mode 'remaining' retourne tout 0 (PO entièrement reçu), afficher quand même les lignes avec remaining=0
        if (
          poQuantityMode === "remaining" &&
          selected.every((l) => (l.qty || 0) <= 0)
        ) {
          // On conserve qty = 0 mais on les laisse passer pour que l'utilisateur visualise les lignes (éviter ligne vide unique)
          selected = rawLines.map((l) => ({
            ...l,
            qty: 0,
            _allRemainingZero: true,
          }));
        }
        // Filtrer lignes avec quantité > 0
        const positive = selected.filter((l) => l.qty > 0.000001);
        const effective = positive.length
          ? positive
          : selected.length
          ? selected
          : [];
        const mapped = effective.map((l) => ({
  ...
  const submit = async (e) => {
    ...
    if (!form.supplierInvoiceNumber.trim()) {
      setError("Numéro facture fournisseur requis");
      return;
    }
    if (!lines.length) {
      setError("Au moins une ligne nécessaire");
      return;
    }
    ...
    try {
      const payload = {
        ...
        lines: lines.map((l) => ({
          ...l,
          purchaseOrderLineId:
            l.purchaseOrderLineId || l._purchaseOrderLineId || undefined,
        })),
        ...
        <form onSubmit={submit} className="flex flex-col gap-6 text-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block mb-1">
                Bon de commande (réception complète requise)
              </label>
              <select
              ...
              {poLoading && (
                <div className="text-xs text-gray-500 mt-1">
                  Chargement lignes du BC...
                </div>
              )}
              {!poLoading && !purchaseOrders.length && (
                <div className="text-[11px] text-yellow-700 mt-1">
                  Aucun bon de commande reçu pour ce fournisseur. Finalisez la
                  réception avant d'enregistrer la facture.
                </div>
              )}
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-gray-700">
                    ...
                    <label className="inline-flex items-center gap-1">
                      <input
                        ...
                        name="poQtyMode"
                        value="received"
                        checked={poQuantityMode === "received"}
                        onChange={() => setPoQuantityMode("received")}
                      />
                      <span>Reçues</span>
                    </label>
                    <label className="inline-flex items-center gap-1">
                      <input
                        ...
                        name="poQtyMode"
                        value="ordered"
                        checked={poQuantityMode === "ordered"}
                        onChange={() => setPoQuantityMode("ordered")}
                      />
                    ...
                    <label className="inline-flex items-center gap-1">
                      <input
                        ...
                        name="poQtyMode"
                        value="remaining"
                        checked={poQuantityMode === "remaining"}
                        onChange={() => setPoQuantityMode("remaining")}
                      />
                      <span>Restantes</span>
                    </label>
                  </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={addLine}
              disabled={autoFromPO}
              className="text-xs bg-green-600 hover:bg-green-700 disabled:opacity-40 text-white px-3 py-1 rounded"
            >
              + Ligne
            </button>
          </div>
                    ...
                    <tr key={idx} className="border-t">
                      <td className="px-2 py-1">
                        <input
                          ...
                          className="border rounded px-2 py-1 w-full disabled:opacity-70"
                          disabled={l._locked}
                        />
                      </td>
                      ...
                      <td className="px-2 py-1">
                        <input
                          ...
                          onChange={(e) =>
                            updateLine(idx, { unitOfMeasure: e.target.value })
                          }
                          className="border rounded px-2 py-1 w-full disabled:opacity-70"
                          disabled={l._locked}
                        />
          ...
          {success && (
            <div className="text-green-600 text-xs">Facture créée ✔</div>
          )}
          ...
          <div className="flex gap-2">
            <button
              disabled={loading}
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white px-5 py-2 rounded"
            >
              Enregistrer
            </button>
            <button
