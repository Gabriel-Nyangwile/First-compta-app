...
export async function getClientLettering({ clientId }) {
  ...
  const items = transactions.map((tx) => {
    ...
    const movement = tx.moneyMovement;
    const amount = toNumber(tx.amount);
    const side =
      tx.kind === "PAYMENT" || movement?.kind === "CLIENT_RECEIPT"
        ? "PAYMENT"
        : "RECEIVABLE";
    ...
    return {
      ...
      accountNumber: tx.account?.number || null,
      accountLabel: tx.account?.label || null,
      invoice: invoice
        ? {
            id: invoice.id,
            ...
            outstanding: toNumber(invoice.outstandingAmount),
          }
        : null,
      movement: movement
        ? {
  ...
  for (const item of items) {
    statusCounts.set(
      item.letterStatus,
      (statusCounts.get(item.letterStatus) || 0) + 1
    );
    if (item.side === "RECEIVABLE") totalReceivable += item.amount;
  ...
  return {
    ...
    totals: {
      ...
      delta: totalReceivable - totalPayment,
    },
    statusCounts: Array.from(statusCounts.entries()).map(([status, count]) => ({
      status,
      count,
    })),
  };
...
export async function getThirdPartyLedger({
  ...
  const baseKinds =
    party === "client" ? ["RECEIVABLE", "PAYMENT"] : ["PAYABLE", "PAYMENT"];
    ...
    include: {
      ...
      moneyMovement: {
        select: {
          ...
          voucherRef: true,
          kind: true,
          authorization: { select: { docNumber: true } },
          bankAdvice: { select: { refNumber: true } },
          moneyAccount: {
            select: {
              ledgerAccount: { select: { number: true, label: true } },
              label: true,
            },
          },
        },
  ...
  for (const g of periodGroups) {
    const amt = g._sum.amount || new Prisma.Decimal(0);
    if (g.direction === "DEBIT") periodDebit = periodDebit.plus(amt);
    else periodCredit = periodCredit.plus(amt);
  }
  ...
  //  * For payments: show ONLY the treasury account line (57/520/53/51x) and exclude 401/411
  ...
  const isExpense = (acctNumber) =>
    acctNumber ? /^6/.test(acctNumber) : false;
  const isRevenue = (acctNumber) =>
    acctNumber ? /^7/.test(acctNumber) : false;
  const isTreasury = (acctNumber) =>
    acctNumber ? /^(57|520|53|51)/.test(acctNumber) : false;
  ...
  for (const t of rows) {
    ...
    if (
      (t.kind === "PAYABLE" || t.kind === "RECEIVABLE") &&
      t.description &&
      /Règlement facture/.test(t.description)
    ) {
      // On ignore la ligne principale 401/411 (utilisée seulement pour delta) -> pas d'ajout de groupe
      continue;
    }
    if (t.incomingInvoice) {
      pushToGroup("INV_SUP:" + t.incomingInvoice.id, t);
      continue;
    }
    if (t.invoice) {
      pushToGroup("INV_CLI:" + t.invoice.id, t);
      continue;
    }
  ...
  let orientedRunning =
    orientation === "CLIENT" ? neutralRunning : neutralRunning.mul(-1);
  const displayRows = [];
  ...
  for (const g of groups) {
    const txs = g.txs;
    const sample = txs[0];
    const isPaymentGroup =
      sample.moneyMovement?.kind === "SUPPLIER_PAYMENT" ||
      sample.moneyMovement?.kind === "CLIENT_RECEIPT";
    const incomingInv =
      txs.find((t) => t.incomingInvoice)?.incomingInvoice || null;
    ...
    if (isPaymentGroup) {
      ...
      const mainTx =
        txs.find((t) => isMainAccount(t.account?.number)) ||
        txs.find(
          (t) => t.kind === (party === "supplier" ? "PAYABLE" : "RECEIVABLE")
        );
      // If a treasury line with supplierId/clientId exists capture it (non main account)
      let treasuryTx = txs.find(
        (t) => !isMainAccount(t.account?.number) && t.kind === "PAYMENT"
      );
      const amountDecimal = mainTx
        ? new Prisma.Decimal(mainTx.amount)
        : treasuryTx
        ? new Prisma.Decimal(treasuryTx.amount)
      ...
      if (mainTx) {
        neutralDelta =
          mainTx.direction === "DEBIT" ? amountDecimal : amountDecimal.mul(-1);
      } else {
        const mains = txs.filter(
          (t) => t.kind === (party === "supplier" ? "PAYABLE" : "RECEIVABLE")
        );
        for (const m of mains) {
          const amt = new Prisma.Decimal(m.amount);
          neutralDelta = neutralDelta.plus(
            m.direction === "DEBIT" ? amt : amt.mul(-1)
          );
        }
      }
      ...
      const treasuryAccountNumber =
        mainTx?.moneyMovement?.moneyAccount?.ledgerAccount?.number ||
        mainTx?.moneyMovement?.moneyAccount?.ledgerAccount?.number;
      const treasuryAccountLabel =
        mainTx?.moneyMovement?.moneyAccount?.ledgerAccount?.label ||
        mainTx?.moneyMovement?.moneyAccount?.ledgerAccount?.label;
      const direction = (() => {
        if (treasuryTx) return treasuryTx.direction;
        // If supplier: payment entry has mainTx DEBIT on 401 => treasury should be CREDIT
        if (party === "supplier")
          return mainTx?.direction === "DEBIT" ? "CREDIT" : "DEBIT";
        // client receipt: mainTx CREDIT on 411 => treasury should be DEBIT
        if (party === "client")
          return mainTx?.direction === "CREDIT" ? "DEBIT" : "CREDIT";
        return "DEBIT";
      })();
      const amountNumber = Number(amountDecimal);
      const description =
        party === "supplier"
          ? incomingInv?.entryNumber ||
            mainTx?.moneyMovement?.authorization?.docNumber ||
            "Règlement"
          : salesInv?.invoiceNumber ||
            mainTx?.moneyMovement?.authorization?.docNumber ||
            "Encaissement";
      const invoiceRef =
        mainTx?.moneyMovement?.voucherRef ||
        mainTx?.moneyMovement?.authorization?.docNumber ||
        mainTx?.moneyMovement?.bankAdvice?.refNumber ||
        mainTx?.moneyMovement?.id;
      addRow({
        id: treasuryTx?.id || mainTx?.id + "-PAY",
        date: treasuryTx?.date || mainTx?.date,
        accountNumber:
          treasuryTx?.account?.number || treasuryAccountNumber || null,
        accountLabel:
          treasuryTx?.account?.label || treasuryAccountLabel || null,
        description,
        ...
        movementId:
          mainTx?.moneyMovement?.id || treasuryTx?.moneyMovement?.id || null,
        paymentRef:
          mainTx?.moneyMovement?.voucherRef ||
          mainTx?.moneyMovement?.authorization?.docNumber ||
          mainTx?.moneyMovement?.bankAdvice?.refNumber ||
          null,
        movementKind:
          mainTx?.moneyMovement?.kind ||
          treasuryTx?.moneyMovement?.kind ||
          null,
        linesPreview: null,
        isPayment: true,
        isVat: false,
    ...
    if (g.key.startsWith("PAYBANK:")) {
      ...
      const mainTx = rows.find(
        (r) =>
          r !== bankTx &&
          r.nature === "payment" &&
          r.kind === (party === "supplier" ? "PAYABLE" : "RECEIVABLE") &&
          ((r.incomingInvoice &&
            bankTx.incomingInvoice &&
            r.incomingInvoice.id === bankTx.incomingInvoice.id) ||
            (r.invoice &&
              bankTx.invoice &&
              r.invoice.id === bankTx.invoice.id)) &&
          r.amount === bankTx.amount
      );
      ...
      let accountNumber =
        bankTx.account?.number ||
        bankTx.moneyMovement?.moneyAccount?.ledgerAccount?.number ||
        null;
      let accountLabel =
        bankTx.account?.label ||
        bankTx.moneyMovement?.moneyAccount?.ledgerAccount?.label ||
        null;
      if (!accountNumber) {
        accountNumber = "5XXX";
      ...
      if (mainTx)
        neutralDelta =
          mainTx.direction === "DEBIT"
            ? new Prisma.Decimal(mainTx.amount)
            : new Prisma.Decimal(mainTx.amount).mul(-1);
      else {
      ...
      const invoiceRef =
        invoiceObj?.entryNumber || invoiceObj?.invoiceNumber || null;
      addRow({
        id: bankTx.id,
        date: bankTx.date,
        accountNumber,
    ...
    if (isSupplierInvoice || isClientInvoice) {
      ...
      if (vatLines.length) {
        ...
        if (mainTx) {
          ...
          for (const nv of nonVatLines)
            nonVatSum = nonVatSum.plus(new Prisma.Decimal(nv.amount).abs());
          ...
          if (expectedTotalVat.greaterThan(0)) {
            let currentVatTotal = vatGroupedLines.reduce(
              (acc, v) => acc.plus(new Prisma.Decimal(v.amount).abs()),
              new Prisma.Decimal(0)
            );
            // Si l'écart est significatif (>1 centime) et current > expected (signe d'un doublon), on rééchelonne proportionnellement
            if (
              currentVatTotal.greaterThan(0) &&
              currentVatTotal
                .minus(expectedTotalVat)
                .greaterThan(new Prisma.Decimal(0.01)) &&
              currentVatTotal.greaterThan(expectedTotalVat)
            ) {
              ...
              for (let i = 0; i < vatGroupedLines.length; i++) {
                ...
                const sign = new Prisma.Decimal(vg.amount).isPositive()
                  ? 1
                  : -1;
      ...
      vatGroupedLines.sort((a, b) =>
        (a._vatGroupAccount || "").localeCompare(b._vatGroupAccount || "")
      );
      ...
      if (mainTx) {
        ...
        for (const l of txs) {
          const n =
            l.direction === "DEBIT"
              ? new Prisma.Decimal(l.amount)
              : new Prisma.Decimal(l.amount).mul(-1);
      ...
      const finalOrientedRunning =
        orientation === "CLIENT"
          ? finalNeutralRunning
          : finalNeutralRunning.mul(-1);
      ...
      for (let i = 0; i < orderedDisplayed.length; i++) {
        ...
        if (!description && baseInv) {
          description =
            incomingInv?.supplierInvoiceNumber ||
            incomingInv?.entryNumber ||
            salesInv?.invoiceNumber ||
            description;
        }

        const invoiceRef = incomingInv
          ? incomingInv.entryNumber || incomingInv.supplierInvoiceNumber || incomingInv.id
          : salesInv
          ? salesInv.invoiceNumber || salesInv.id
          : null;
        const invoiceStatus = salesInv?.status || incomingInv?.status || null;
        const invoiceDueDate = salesInv?.dueDate || incomingInv?.dueDate || null;
        const orientedRunForRow = isLast
          ? Number(finalOrientedRunning)
          : Number(
              orientation === "CLIENT"
                ? preRunningNeutral
                : preRunningNeutral.mul(-1)
            );

        addRow({
          id: line.id || `${sample.id}-INV-${i}`,
          date: line.date || sample.date,
          ...
          invoiceStatus,
          invoiceDueDate,
          running: orientedRunForRow,
          movementId: sample.moneyMovement?.id || null,
          paymentRef:
            sample.moneyMovement?.voucherRef ||
            sample.moneyMovement?.authorization?.docNumber ||
            sample.moneyMovement?.bankAdvice?.refNumber ||
            null,
    ...
    for (const t of txs) {
      const neutralDelta =
        t.direction === "DEBIT"
          ...
          : new Prisma.Decimal(t.amount).mul(-1);
      neutralRunning = neutralRunning.plus(neutralDelta);
      orientedRunning =
        orientation === "CLIENT" ? neutralRunning : neutralRunning.mul(-1);
      addRow({
        ...
        invoiceRef:
          t.invoice?.invoiceNumber || t.incomingInvoice?.entryNumber || null,
        invoiceStatus: t.invoice?.status || t.incomingInvoice?.status || null,
        invoiceDueDate:
          t.invoice?.dueDate || t.incomingInvoice?.dueDate || null,
        running: Number(orientedRunning),
        movementId: t.moneyMovement?.id || null,
        paymentRef:
          t.moneyMovement?.voucherRef ||
          t.moneyMovement?.authorization?.docNumber ||
          t.moneyMovement?.bankAdvice?.refNumber ||
          null,
  ...
  if (party === "supplier") {
    ...
    for (const r of ledgerRows) {
      if (!r.invoiceRef) continue;
      if (!byRef.has(r.invoiceRef)) byRef.set(r.invoiceRef, []);
      byRef.get(r.invoiceRef).push(r);
    }
  ...
  if (party === "supplier") {
    const hadPaymentTx = rows.some(
      (r) =>
        r.kind === "PAYMENT" &&
        r.account?.number &&
        /^(57|520|53|51|521|522)/.test(r.account.number)
    );
    const displayedPayment = ledgerRows.some((r) => r.kind === "PAYMENT");
    if (hadPaymentTx && !displayedPayment) {
      console.warn(
        "[LEDGER] Paiement détecté dans rows mais aucune ligne PAYMENT affichée (supplierId=" +
          id +
          ")."
      );
    }
  ...
  if (party === "client") {
    ...
    if (s) {
      partyName = s.name;
      partyAccountNumber = s.account?.number || null;
      partyAccountLabel = s.account?.label || null;
      partyMeta = {
        email: s.email,
        paymentDelay: s.paymentDelay,
        paymentNature: s.paymentNature,
      };
    }
  }
...
export async function getClientLedger(params) {
  return getThirdPartyLedger({ party: "client", ...params });
}
...
export async function getSupplierLettering({ supplierId }) {
  ...
  const transactions = await prisma.transaction.findMany({
    where: {
      OR: [
        ...
        { moneyMovement: { supplierId } },
      ],
      kind: { in: ["PAYABLE", "PAYMENT"] },
    },
    orderBy: { date: "asc" },
    include: {
      account: { select: { number: true, label: true } },
  ...
  const items = transactions.map((tx) => {
    const invoice = tx.incomingInvoice;
    const movement = tx.moneyMovement;
    const amount = toNumber(tx.amount);
    const side =
      tx.kind === "PAYMENT" || movement?.kind === "SUPPLIER_PAYMENT"
        ? "PAYMENT"
        : "PAYABLE";
  ...
  for (const item of items) {
    ...
    if (item.side === "PAYABLE") totalPayable += item.amount;
    if (item.side === "PAYMENT") totalPayment += item.amount;
  }
  ...
  return {
    ...
    totals: {
      ...
      payment: totalPayment,
      delta: totalPayable - totalPayment,
    },
    statusCounts: Array.from(statusCounts.entries()).map(([status, count]) => ({
      status,
      count,
