...
// Composant de lettrage client (analogue à LetteringPanel fournisseur)
...
async function fetchLettering(clientId) {
  const res = await fetch(`/api/clients/${clientId}/lettering`, {
    cache: "no-store",
  });
...
async function triggerMatch({ clientId, movementId }) {
  const res = await fetch(`/api/clients/${clientId}/lettering/match`, {
    method: "POST",
...
async function triggerInvoiceMatch({ clientId, invoiceId }) {
  const res = await fetch(`/api/clients/${clientId}/lettering/match-invoice`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export function ClientLetteringPanel({ clientId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setError(null);
        const payload = await fetchLettering(clientId);
  ...
  const handleMatch = (movementId) => {
    ...
    startTransition(async () => {
      try {
        const result = await triggerMatch({ clientId, movementId });
        setMessage(
        ...
        const refreshed = await fetchLettering(clientId);
        setData(refreshed);
      } catch (err) {
  ...
  const totals = data?.totals || { receivable: 0, payment: 0, delta: 0 };
      ...
      <div className="px-4 py-3 border-b flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Suivi des écritures 411/520/57 associées à ce client.
          </p>
        </div>
        <div className="text-xs text-gray-600 flex flex-col md:flex-row gap-2 md:items-center">
          <span>
            ...
            <span className="font-semibold ml-1">
              <Amount value={totals.receivable} />
            </span>
          </span>
          <span>
            Somme paiements&nbsp;:
      ...
      <div className="px-4 py-3 border-b space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-gray-500">
              Commencez par la facture, puis laissez le système imputer les encaissements encore disponibles.
            </p>
          </div>
          <div className="text-[11px] text-gray-600 text-right">
            <div>
              Encaissements encore disponibles :{" "}
        ...
        {availablePayments.length === 0 && (
          <div className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] text-amber-700">
            Aucun encaissement disponible à imputer. Enregistrez d&apos;abord un encaissement client avant de lancer le lettrage.
          </div>
        )}
              ...
              {invoices.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-4 text-center text-gray-400">
                    Aucune facture client trouvée pour ce lettrage.
                  </td>
                </tr>
              )}
                      ...
                      <button
                        ...
                        title={
                          invoice.letterStatus === "MATCHED"
                            ...
                            : availablePayments.length === 0
                              ? "Aucun encaissement disponible pour cette facture."
                              : "Imputer les encaissements disponibles sur cette facture."
                        }
                        onClick={() => {
                          setMessage(null);
                          setDetail(null);
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={8} className="px-4 py-6 text-center text-gray-400">
                  Aucune écriture 411/520/57 trouvée pour ce client.
                </td>
              </tr>
            )}
            {items.map((row) => {
