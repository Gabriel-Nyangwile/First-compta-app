...
async function main() {
  ...
  const counterpart = await prisma.account.create({ data: { number: '471TEST'+Date.now(), label: 'Compte attente test trésorerie' } });
  ...
  const m1 = await createMoneyMovement({ moneyAccountId: acc.id, amount: 50, direction: 'IN', kind: 'OTHER', description: 'Encaissement test', counterpartAccountId: counterpart.id });
  console.log('Mouvement 1', m1.id);
  const m2 = await createMoneyMovement({ moneyAccountId: acc.id, amount: 30, direction: 'OUT', kind: 'OTHER', description: 'Décaissement test', counterpartAccountId: counterpart.id });
